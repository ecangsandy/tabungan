@extends('layout.new')
	...
	<div class="box">
		<div class="box-title">
			<h2>kridit Nasabah</h2>
		</div>
		<div class="box-body">
			<form class="" action="{{ route('kridit.store') }} " method="POST">
				{{ csrf_field() }}
				...
				<div class="form-group{{ ($errors->has('kridit')) ? $errors->first('kridit') : '' }}">
				<label for="">Nominal </label>
				<input type="text" name="kridit" class="form-control" placeholder="Masukan Nominal Rupiah">
				{!! $errors->first('kridit', '<p class="help-block">:message </p>') !!}
				</div>
